...
// unreachableFloorKeys builds keys whose deep shared prefixes force large
// (~1400-byte) branch separators: a 4 KB branch then fits only ~2 of them
// (fanout 2, ~35% fill), and a delete's redistribute lifts one to the parent
// so both halves stay below MergeThreshold 50 — the fill-floor is genuinely
// unreachable (range-delete.md §Invariants "where reachable"). 6 clusters ×
// 12 keys + large values → a depth->=3 tree of mostly-below-MT branches.
...
// the floor is unreachable here by construction — the architectural fix is
// within-branch prefix compression, tracked separately.
