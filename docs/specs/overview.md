...
| Cross-process | Shared memory lock file (`structs.HostLayout` structs) + per-slot kernel locks | C ABI layout guarantee for mmap'd structs; fixed-size reader table (lock-then-store); reader liveness = held slot lock (probe-and-clear reclamation, no identity/heartbeat/timer); writer records keep PID + start time + namespace + heartbeat for the recovery-commit gate |
...
| Branch keys | Cross-level-truncated separators; two per-keyspace layouts: plain (full separators, latency floor) and segregated (page prefix once + suffix heap + segregated child pointers, default) | Shortest distinguishing prefix across levels; the segregated layout stores a page's shared separator prefix once, so fan-out stays high even for deep-shared-prefix keys; shallow trees; full keys in leaves only |
| Leaf compression | Three variants: segregated prefix-compressed (default; keys and values in separate regions), interleaved prefix-compressed, and uncompressed leaves (`RestartGroupTarget = 1`) | Density gains for shared-prefix workloads (directory listings, composite keys); the segregated variant keeps searches in a pure key region and speeds full-page decode; interleaved favors in-place value splices for write-heavy keyspaces; the uncompressed variant trades compression for single-O(log N) lookup, O(1) `Prev`, and simpler `Check()` walks |
...
| Checksums | Unified XXH3-64: 8-byte footers on meta and node/RPL pages, one head-resident whole-run digest per overflow run; on by default | One hash family across the file; software-fast (benchmark-favored over CRC32C); defense against silent bitrot on commodity filesystems; footer-free overflow runs keep extent bytes contiguous for single-slice borrowed values |
