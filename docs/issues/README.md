...
| [inv-tag-audit](inv-tag-audit.md) | proactive | **[M]** ~39 distinct `Inv-XXX` coded-invariant tags (~100+ uses) in prod comments. Most dangle — specs use unlabeled `Invariant:` blocks, so only `Inv-IHS1/2/3`, `Inv-N3`, `Inv-M4` are real cross-refs. Inline the property or repoint to the spec doc/§ for the rest; keep the real ones. Split from dev-process-artifacts after the chunk-N scrub landed. |
