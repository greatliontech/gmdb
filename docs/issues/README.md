...
| [btree-splice-layer](btree-splice-layer.md) | proactive (decide format-alignment fork first) | **[perf]** Build the in-place leaf splice layer (no-decode insert/delete/split) the spec describes but gmdb never built (grove has it, wired). Profile: ~26% of put time is leaf decode + ~50 allocs/op; splice est. ~25-35% faster, ~5-10× fewer allocs. Port grove's *algorithm* — but gmdb/grove compressed formats **differ** (inline `ValueLen` position), so a verbatim port corrupts; decide align-vs-adapt first. Multi-session. |
