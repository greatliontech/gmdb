...
| [keyspace-setkeyspace-shared-core](keyspace-setkeyspace-shared-core.md) | proactive | **[M]** `Keyspace`/`SetKeyspace` are parallel hierarchies with no shared base; ~120–150 lines of byte-identical helpers (`markIndexHandle*`, `builderCfg`, `markDirty`, cursor-factory ×4, guard preambles ×12). Extract a `keyspaceCore` (the `Index` nil-union is the in-repo template). |
