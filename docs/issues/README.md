...
— leaving the twelve below. The `[H]`/`[M]`/`[L]` tags in their
...
| [keyspace-setkeyspace-shared-core](keyspace-setkeyspace-shared-core.md) | proactive | **[M, partial]** `keyspaceCore` embed landed (8 shared fields + 7 byte-identical helpers consolidated, −315 lines, behavior-preserving). Remaining: cursor-construction factory (×4) + write-guard preamble (×12) + DeleteRange/BulkLoad preamble tails. |
