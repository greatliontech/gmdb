...
| [descriptor-drift-on-partial-failure](descriptor-drift-on-partial-failure.md) | when tx-poison semantics are added (5.6 or focused recovery chunk) | `Keyspace.Put`/`Delete`/`Cursor.Delete` mutate the data B+tree before the descriptor write; if `storeDescriptor` fails (`ErrTxTooLarge`) the user can Commit and produce on-disk orphan pages. Surfaced by the chunk-5.5 Round 1 adversarial review (H1). |
