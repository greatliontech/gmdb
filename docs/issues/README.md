...
| [index-handle-stale-after-deletekeyspace](index-handle-stale-after-deletekeyspace.md) | opportunistically — single-line guards at each `*Index` entry method mirroring the existing `idx.dead` checks | `transactions.md §Cursor invalidation by DeleteKeyspace` says `tx.DeleteKeyspace(name)` invalidates `*Index` handles → `ErrKeyspaceClosed`, but `*Index.Lookup`/`LookupKeys`/`Range`/`Prefix`/`Get`/`Stats` don't check `idx.ks.dead` / `idx.sks.dead`. After DeleteKeyspace, `retireIndexRegistry` FreeSubtree's the index data trees, so cached `*Index.Stats()` returns the stale pre-delete `Count` and a fresh `idx.Lookup` walks freed root pages. Pre-existing adjacent gap to the closed `index-handle-stale-after-rebuild-drop` (same fault class, different cause-line). Filed-and-proceed at the close-out. |
