...
| [rplchain-head-tail-terminology-conflict](rplchain-head-tail-terminology-conflict.md) | when RPL chain / `reclaimRPL` / `trimRPLChainHead` is next non-trivially touched, or opportunistically | `pager.go:369-370` defines chain orientation as `tail (index 0, oldest) → head (last, newest)`, but `trimRPLChainHead` pops `p.rplSegments[0]` — actually trimming the chain *tail* per the convention, while its name says "head". Pre-existing inconsistency; pure docs/naming cleanup, no correctness implication. Filed at the `rplsegments-clone-cost` close-out (Round-1 adversarial review M-1, adjacent). |
