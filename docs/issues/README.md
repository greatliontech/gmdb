...
| [cross-process-writer-stale-state](cross-process-writer-stale-state.md) | proactive | **[H]** In-memory meta/bitmap/RPL are loaded only at `Open` and never re-synced after a peer process commits → guaranteed lost-update + page-aliasing corruption for serialized cross-process writers; dual metas with equal TxnID. **Scope DECIDED (2026-05-31): multi-process write IS in v0 scope → fix.** Fix: re-sync meta/bitmap/RPL on write-grant acquisition in `Begin`/`BeginRead` (factor `reopenAfterCompact`); acceptance-gated by the cross-process two-handle test. *Subsumes findings 6,8.* |
...
| [rpl-reclamation-checkpoint-bound](rpl-reclamation-checkpoint-bound.md) | proactive | **[H] — CONFIRMED (2026-05-31)** Reclamation bound uses `prevMeta.TxnID`, not the spec's `lastCheckpointTxnID`; under SyncLazy it frees RPL segments a still-recoverable meta's chain references → reopen fails (`malformed RPL segment`), **DB unopenable**. Dispute RESOLVED by the decisive test (`TestRPLSyncLazyReopenCorruptionMinimal`, skipped pending fix; `bound=0` confirms the cause). Fix is DEEPER than the spec: `free-space.md`'s `lastCheckpointTxnID` bound is **itself insufficient** (implemented exactly → still corrupts 30/30, because a recoverable checkpoint's chain tail references segments tagged below its TxnID); reclamation must not free a segment **page** any recoverable meta's chain references — **spec amend needed**. *Finding 4.* |
