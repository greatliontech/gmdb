...
| [writenewindexregistry-partial-leak](writenewindexregistry-partial-leak.md) | when the engine's rest-of-tx-continues-after-error contract is canonicalized (chunk 11 free-space recovery, or whenever loose-pages-on-error policy is reconsidered) | `writeNewIndexRegistry` calls `registryPut` per declared IndexDecl; failure on iteration `k+1` leaves iterations 0..k's pages reachable only via the uncached *Keyspace.desc.IndexRegistryRoot — `Tx.Rollback` reclaims, but `Tx.Commit` (per the rest-of-tx-continues contract) commits with the orphaned pages. Adjacent to chunk-7.8 RebuildIndex's similar loop shape. Surfaced by chunk-7.5 Round-1 adversarial review (M-3). |
