...
| [lock-clear-before-unlock-ordering-test](lock-clear-before-unlock-ordering-test.md) | 2.6 | Direct enforcement test for the chunk-2.4 spec-amended "clear-before-unlock" invariant. Needs stale-writer-recovery's peer-thread infrastructure (or a release-hook test injection) to observe the post-`LOCK_UN` header state. |
