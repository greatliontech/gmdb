...
| [btree-branch-rebalance-unreachable-floor](btree-branch-rebalance-unreachable-floor.md) | proactive | **[H]** Pre-existing (2026-05-31): the branch delete-rebalance loop (`rebalanceChildAtPos`/`cousinRebalanceBranch`) has no give-up path when the fill-floor is *unreachable* (large ~1400-byte separators → near-fanout-2 branches below a high `MergeThreshold`). A heavy delete of a depth-≥3 tree at `MergeThreshold 50` **double-frees pages on HEAD** (bitmap corruption); with the byte-balanced branch redistribute it **never terminates / OOMs**. Separate root cause from byte-balanced-split (the rebalance loop, not the boundary choice); reproduces on HEAD `be63f47` with count-midpoint. |
